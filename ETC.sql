...
SELECT 사번, 사원이름, IFNULL(부서명,'부서없음') FROM employee_view;
...
-- LEFT(문자열, 길이), RIGHT(문자열, 길이) : 문자열을 한쪽에서부터 추출
SELECT LEFT(사원이름, 1)'성', RIGHT(사원이름, 2) '이름' FROM employee_view;

-- REPACE(기준문자열, 찾을문자열, 바꿀문자열) : 특정 문자열을 지정한 문자열로 변경하는 함수
