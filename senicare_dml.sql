...
-- 요양사 테이블에 레코드(*아이디, 암호화된 비밀번호, 전화번호, 가입경로,이름 )을 기입해야함
-- 필요한 추가 데이터 : 가입 경로 (화면에서 가져옴) 암호화된 비밀번호 ()
...
-- 2. 이미 인증 절차가 진행중인 전화번호가 있다면 해당 전화번호 인증 번호를 수정
UPDATE tel_auth SET auth_number = '0123' WHERE '01012345678';
...
-- 고객 테이블에서 페이지 번호에 해당하는 위치부터 10개의 고객 수를 반환
...
-- 고객 테이블에서 이름이 포함되어 있는 레코드들을 페이지 번호에 해당하는 위치부터 10개의 고객을 내림차순 변환 
...
-- 고객 상세보기
-- 최종 사용자 (고객 정보) => (고객번호, 고객 사진, 고객 이름, 생년 월일, 담당자, 담당자 이름, 주소)
